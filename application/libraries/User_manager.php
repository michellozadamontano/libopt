...
class User_manager {
	...
	* @author Waldir Bertazzi Junior
	**/
	function save_user($full_name, $login, $password, $email,$cargo, $active = 1, $permissions=array()) {
		...
		if( ! $this->login_exists($login) && $full_name!= "") {
			...
			if ( $this->CI->db->insert('users', array('name'=>$full_name, 'login'=>$login, 'email'=>$email,'cargo'=>$cargo, 'password'=>$hashed_password, 'active'=>$active )) ) {
    ...
    //esta funcion me devuelve el tipo de permiso que tiene el usuario.
    function tye_of_permission($user_id){
        $result = $this->CI->db->get_where('users_permissions', array('user_id' => $user_id))->row();
        return $result;
    }
